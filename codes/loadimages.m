function IMGDB = loadimages(p)

if p == 1
    face_folder = 'face/';  %LOCATION OF FACE IMAGES
    non_face_folder = 'non-face/'; %LOCATION OF NON-FACE IMAGES    
else
    face_folder = uigetdir(cd,'Select folder contain face images');
    face_folder = strcat(face_folder,'\');
    non_face_folder = uigetdir(cd,'Select folder contain non-face images');
    non_face_folder = strcat(non_face_folder,'\');
end
file_ext = '.png';
